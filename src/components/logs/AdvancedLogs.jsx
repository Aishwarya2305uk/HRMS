...
/**
 * Advanced Logs — the TECHNICAL half of the audit story: one table row per API
 * call, newest first, with the failure reason on every unsuccessful one.
 *
 * Unchanged in behaviour from when this was the whole System Logs page; it now
 * renders as the second tab beside Activity Logs (see SystemLogs.jsx), so the
 * outer <section className="card"> and the page <h1> live in the shell and
 * this component starts at its own description line.
 *
 * Self-contained (owns its fetch state rather than using useAsyncData) because
 * the filter set drives the query and search needs a debounce.
 */
export default function AdvancedLogs() {
    ...
    <>
      <div className="syslog__head">
        <p className="syslog__sub">
          Every API call — success, failure and crash — kept in our own database for 30 days,
          so nothing is lost on redeploy. Form values, documents, query strings and tokens are
          never recorded.
        </p>
